...
import { selectApi, setValueIndex } from '@/components/ui/WcSelect/composables.js';
...
const api = (props, emit) => {
  ...
  const { onKeyDown } = keyEvents(allOptions, emit, highlightedIndex, props.autocomplete, selectedIndex, listboxOpen);
...
const keyEvents = (options, emit, highlightedIndex, autocomplete, selectedIndex, listboxOpen) => {
  ...
  const onKeyDown = (e) => {
    ...
    if (keyCodes.up === keyCode) {
      listboxOpen.value && prevValue(lastIndex.value, highlightedIndex, selectedIndex, listboxOpen);
    }
    if (keyCodes.down === keyCode) {
      listboxOpen.value && nextValue(lastIndex.value, highlightedIndex, selectedIndex, listboxOpen);
    }
...
const nextValue = (lastIndex, highlightedIndex, selectedIndex, listboxOpen) => {
  ...
  highlightedIndex.value = selectedIndex.value < lastIndex ? selectedIndex.value + 1 : 0;
};

const prevValue = (lastIndex, highlightedIndex, selectedIndex, listboxOpen) => {
  openListbox(listboxOpen);

  highlightedIndex.value = selectedIndex.value > 0 ? selectedIndex.value - 1 : lastIndex;
};

const setSearchedValue = (options, search) => {
  return options.value.findIndex((option) => getValueName(option).toLowerCase().startsWith(search));
};
